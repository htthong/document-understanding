...
def compute_edit_distance_matrix_fast(gt_lines, matched_lines):
    """Faster drop-in for compute_edit_distance_matrix_new using rapidfuzz.

    Two shortcuts over the original:
    - length-ratio guard: if one string is >4x the other, distance ≥ 0.75 so
      we assign 1.0 without calling Levenshtein at all.
    - score_cutoff=0.99: rapidfuzz bails early once it knows the answer is ≥ 0.99,
      avoiding full DP for pairs that clearly won't match.
    """
    ...
    for i, gt_line in enumerate(gt_lines):
        ...
        for j, matched_line in enumerate(matched_lines):
            len_m = len(matched_line)
            if len_gt == 0 and len_m == 0:
                distance_matrix[i][j] = 0.0
            elif len_gt == 0 or len_m == 0:
                distance_matrix[i][j] = 1.0
            elif min(len_gt, len_m) * 4 < max(len_gt, len_m):
                distance_matrix[i][j] = 1.0
            else:
                distance_matrix[i][j] = _rflev.normalized_distance(
                    gt_line, matched_line, score_cutoff=0.99
                )
...
def match_gt2pred_quick(gt_items, pred_items, line_type, img_name, skip_truncated=False):
    ...
    if len(ignore_gt_lines) > 0:
        ...
        ignore_matrix = compute_edit_distance_matrix_fast(ignore_gt_lines, norm_pred_lines)
        # print("-------------ignore_matrix-------------")
        # print(ignore_matrix)
    ...
    cost_matrix = compute_edit_distance_matrix_fast(no_ignores_gt_lines, no_ignores_pred_lines)
    # print("-------------cost matrix-------------")
    # print(cost_matrix)

    matched_col_idx, row_ind, cost_list = cal_final_match_fast(cost_matrix, no_ignores_gt_lines, no_ignores_pred_lines, skip_truncated=skip_truncated)
...
_FUZZY_MAX_LEN = 300  # skip sliding-window for strings longer than this
...
def sub_pred_fuzzy_matching(gt, pred):
    ...
    if gt_len > _FUZZY_MAX_LEN or pred_len > _FUZZY_MAX_LEN:
        return False
    ...
    if gt_len >= pred_len and pred_len > 0:
        for i in range(gt_len - pred_len + 1):
            ...
            if dist < min_d:
                min_d = dist
                pos = i
            if min_d < 0.05:  # early exit: found a near-perfect match
                break
...
def deal_with_truncated(cost_matrix, norm_gt_lines, norm_pred_lines):
    ...
    new_cost_matrix = compute_edit_distance_matrix_fast(norm_gt_lines, final_norm_pred_lines)
...
def cal_final_match_fast(cost_matrix, norm_gt_lines, norm_pred_lines, skip_truncated=False):
    """Like cal_final_match but skips deal_with_truncated for large matrices.

    deal_with_truncated is O(N_unmatched * M_unmatched * string_len) and becomes
    prohibitively slow for multipage windows (100x100+ matrices). Above 3600
    entries (60x60) we fall back to plain linear_sum_assignment, which is the
    same result deal_with_truncated produces when it finds nothing to merge.
    Pass skip_truncated=True to always skip it (e.g. for multipage datasets).
    """
    if not skip_truncated and cost_matrix.size <= 3600:
        new_cost_matrix, _, final_pred_idx_list = deal_with_truncated(cost_matrix, norm_gt_lines, norm_pred_lines)
    else:
        new_cost_matrix = cost_matrix
        final_pred_idx_list = range(len(norm_pred_lines))
...
def fuzzy_match_unmatched_items(unmatched_gt_indices, norm_gt_lines, norm_pred_lines):
    ...
    for pred_idx, pred_content in enumerate(norm_pred_lines):
        if isinstance(pred_idx, list):
            continue
        if len(pred_content) > _FUZZY_MAX_LEN:
            continue
        ...
        for unmatched_gt_idx in unmatched_gt_indices:
            ...
            if len(gt_content) > _FUZZY_MAX_LEN:
                continue
...
def convert_final_matches(final_matches, norm_gt_lines, norm_pred_lines):
    ...
    if unmatched_pred_indices:
        if unmatched_gt_indices:
            unmatched_gt_list   = [norm_gt_lines[i]   for i in unmatched_gt_indices]
            unmatched_pred_list = [norm_pred_lines[i] for i in unmatched_pred_indices]
            distance_matrix = compute_edit_distance_matrix_fast(unmatched_gt_list, unmatched_pred_list)
