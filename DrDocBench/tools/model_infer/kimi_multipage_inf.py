"""
Multi-page sliding-window Kimi-K2.5 img2md inference.

Note: Kimi-K2.5 uses <|media_begin|> (not <|media_start|>) in its chat template.
This only matters for local vLLM serving; the official API handles it server-side.
...
_CONTENT_FILTER_PHRASES = ("Input data may contain inappropriate content.",)
...
def main():
    parser = argparse.ArgumentParser(description="Multi-page sliding-window Kimi-K2.5 inference for document images")
