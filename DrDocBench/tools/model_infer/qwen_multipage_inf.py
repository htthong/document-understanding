...
_CONTENT_FILTER_PHRASES = ("DataInspectionFailed", "may contain inappropriate")

_rate_limiter = None  # set in main() when --rps is provided
...
class _TokenBucket:
    """Simple token bucket: allows at most `rps` requests per second."""
...
def get_response(image_paths, prompt, model, max_retries=3):
    ...
    `max_retries` times with exponential backoff + full jitter.
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if attempt == max_retries:
                raise
            # Full jitter: uniform in [0, 2^(attempt+1)] seconds, capped at 60s
...
def main():
    parser = argparse.ArgumentParser(description="Multi-page sliding-window Qwen inference for document images")
    parser.add_argument("--image_root", type=str, required=True,
                        help="Root folder containing subject/doc_id/images/ subfolders")
    parser.add_argument("--save_root", type=str, required=True,
                        help="Folder to save markdown results (include the model name for clarity)")
    parser.add_argument("--model", type=str, required=True, help="Model name (e.g. qwen3-vl-235b-a22b-instruct or qwen3.5-397b-a17b)")
    parser.add_argument("--threads", type=int, default=50, help="Number of parallel threads")
    ...
    parser.add_argument("--max-retries", type=int, default=20,
                        help="Max in-request retries for transient API errors (429, 5xx, timeout) "
                             "with exponential backoff + jitter. Permanent errors (e.g. 400) are never retried.")
    parser.add_argument("--rps", type=float, default=10.0,
                        help="Max requests per second across all threads (token-bucket rate limiter). "
                             "Lower this if you hit burst-rate errors.")
