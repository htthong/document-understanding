...
class End2EndEval():
    def __init__(self, dataset, metrics_list, page_info_path, save_name):
        ...
        # Multipage datasets aggregate per-window page_info during loading;
        # prefer that over the file-based dict. The aggregated form is
        # {window_img_name → {page_no → page_attribute}}, and get_page_split
        # detects the multipage shape via _source_page_no on each block.
        if getattr(dataset, 'page_info_aggregated', None):
            page_info = dataset.page_info_aggregated
        ...
        for element in metrics_list.keys():
            ...
            # Always attempt aggregation. Both helpers short-circuit cleanly when
            # there's nothing to aggregate (pure md2md without page_info, or
            # samples without gt_attribute). Multipage path: page_info comes
            # from dataset.page_info_aggregated and get_page_split looks up
            # each block's source page via its `_source_page_no` tag.
            ...
            try:
                ...
                with open(f'./result/{save_name}_{element}_result.json', 'w', encoding='utf-8') as f:
                    json.dump(saved_samples, f, indent=None, ensure_ascii=False)
